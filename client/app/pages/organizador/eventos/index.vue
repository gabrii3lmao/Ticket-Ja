  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <NuxtLink to="/organizador/eventos/novo">
        <UButton color="primary" icon="i-lucide-plus" label="Novo Evento" />
      </NuxtLink>
    </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-800">
              ...
              <th class="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400">Local</th>
          ...
          <tbody>
            ...
            <tr v-else-if="events.length === 0">
              <td colspan="5" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400">
                Nenhum evento encontrado. <NuxtLink to="/organizador/eventos/novo" class="text-brand-600 hover:underline">Criar primeiro evento</NuxtLink>
              </td>
            </tr>
            <tr v-else v-for="event in events" :key="event.id" class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
              <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ event.name }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ formatDate(event.startDate) }}</td>
              <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ event.venue?.name || '—' }}</td>
              <td class="px-4 py-3"><StatusBadge :status="event.status" /></td>
              <td class="px-4 py-3 text-right">
                <NuxtLink :to="`/organizador/eventos/${event.id}`">
